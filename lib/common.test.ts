...
test("format - preserves Unicode pairs and rejects lone surrogates in keys", (t) => {
  t.is(
    format("service🚀", "info", "hello😀", {
      "key🚀": "value😀",
      "bad\ud800": "ignored",
      "bad\udfff": "ignored",
      "bad\ud800x": "ignored",
    ...
    "level=info domain=service🚀 msg=hello😀 key🚀=value😀",
  );
});

test("format - rejects ASCII controls in keys and quotes them in values", (t) => {
  for (const code of [...Array(33).keys(), 127]) {
    const character = String.fromCharCode(code);
    t.is(
      format(undefined, "info", "safe", {
        [`bad${character}key`]: "ignored",
        value: `a${character}b`,
      }),
      `level=info msg=safe value=${JSON.stringify(`a${character}b`)}`,
    );
  }
});
