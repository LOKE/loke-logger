...
function counterFor(registry: MetricsRegistry): CounterConstructor {
  let fallback: CounterConstructor | undefined;
  ...
  for (const name of clientPackages) {
    ...
    const client = requirePeer(name) as PrometheusClient;
    if (registry instanceof client.Registry) return client.Counter;
    fallback ??= client.Counter;
  }
